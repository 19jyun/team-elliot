// src/contexts/forms/PrincipalPersonManagementFormManager.ts
...
export interface PrincipalPersonManagementFormState {
...
export class PrincipalPersonManagementFormManager {
  private state: PrincipalPersonManagementFormState;
  private eventBus: ContextEventBus;
  private listeners: Set<(state: PrincipalPersonManagementFormState) => void> =
    new Set();
  ...
  // 공개 API
  getState(): PrincipalPersonManagementFormState {
    return { ...this.state };
  }
  ...
  // 구독/구독 해제
  subscribe(
    listener: (state: PrincipalPersonManagementFormState) => void
  ): () => void {
  ...
  // 내부 구현 (캡슐화됨)
  private getInitialState(): PrincipalPersonManagementFormState {
    return {
  ...
  private emitStateChange(): void {
    this.eventBus.emit("formStateChanged", {
      formType: "principalPersonManagement",
