...
export function useApiError(): UseApiErrorReturn {
  ...
  const clearErrorAfterDelay = useCallback(
    (fieldName: string, delay: number = 3000) => {
      setTimeout(() => {
        setFieldErrors((prev) => {
          ...
          if (error.code) {
            switch (error.code) {
              ...
              case "ACADEMY_CODE_NOT_FOUND":
                if (!disableToast) {
                  toast.error("존재하지 않는 학원 코드입니다.");
                }
                setFieldErrors({ code: "존재하지 않는 학원 코드입니다." });
                clearErrorAfterDelay("code", 3000); // 3초 후 자동 제거
                break;
              ...
              case "STUDENT_ALREADY_JOINED":
                ...
                setFieldErrors({ code: "이미 가입된 학원입니다." });
                clearErrorAfterDelay("code", 3000); // 3초 후 자동 제거
                break;
  ...
  const handleApiError = useCallback(
    (
      ...
      if (error && typeof error === "object" && "type" in error) {
        // 이미 AppError 형태인 경우
        handleError(error as AppError, options);
      } else {
        // 일반적인 에러인 경우
