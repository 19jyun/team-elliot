...
import type { SessionEnrollment } from "@/types/api/class";
...
export function useRejectEnrollment() {
  ...
  return useMutation({
    ...
    onMutate: async ({ enrollmentId }) => {
      ...
      const previousEnrollments = queryClient.getQueryData<SessionEnrollment[]>(
        queryKeys.principal.enrollments.lists()
      );

      // 낙관적 업데이트
      queryClient.setQueryData<SessionEnrollment[]>(
        queryKeys.principal.enrollments.lists(),
        (old) => {
          if (!old) return old;
          return old.map(
