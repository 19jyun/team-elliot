...
import type { PrincipalSessionEnrollment } from "@/types/api/principal";
...
export function useApproveEnrollment() {
  ...
  return useMutation({
    ...
    onMutate: async (enrollmentId) => {
      ...
      // 낙관적 업데이트
      queryClient.setQueryData<GetPrincipalAllSessionsResponse>(
        queryKeys.principal.enrollments.lists(),
        (old) => {
          if (!old) return old;
          return old.map((session) => ({
            ...session,
            enrollments: session.enrollments.map(
              (enrollment: PrincipalSessionEnrollment) =>
