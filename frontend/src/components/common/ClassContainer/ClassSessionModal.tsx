...
export function ClassSessionModal({ 
  ...
  const { data: sessionEnrollmentsResponse, isLoading: enrollmentsLoading } = useQuery({
    queryKey: ['session-enrollments', selectedSession?.id, role],
    queryFn: () => {
      if (role === 'teacher') {
        return getSessionEnrollments(selectedSession?.id || 0)
      } else {
        return getPrincipalSessionEnrollments(selectedSession?.id || 0)
      }
    },
    ...
    >
      ...
      {selectedSession && (
        <SessionEnrollmentList
          sessionEnrollments={sessionEnrollmentsResponse?.enrollments || []}
