...
export function SignupTermsPage() {
  ...
  return (
    <div className="flex flex-col w-full h-full">
      ...
      {/* Main Content - 고정 높이 + 스크롤 (약관 체크박스 영역) */}
      <main className="flex-shrink-0 overflow-y-auto px-5" style={{ height: 'calc(100vh - 320px)' }}>
        <form id="signup-terms-form" onSubmit={handleSubmit(onSubmit)} className="flex flex-col mt-4 w-full pb-4">
        <button
          type="button"
      ...
      {/* Footer - 고정 높이 (가입완료 버튼) */}
      <footer className="flex-shrink-0 border-t border-gray-200 px-5 py-4 w-full">
        <button
          type="submit"
          form="signup-terms-form"
          disabled={isLoading || !allChecked}
          className="w-full h-11 px-8 inline-flex items-center justify-center gap-2 rounded-md text-sm font-medium text-white bg-primary hover:bg-primary/90 transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:opacity-50"
        >
          {isLoading ? '처리중...' : '가입완료'}
        </button>
      </footer>
