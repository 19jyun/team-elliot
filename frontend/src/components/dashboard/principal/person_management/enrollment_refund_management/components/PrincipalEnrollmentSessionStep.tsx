...
export function PrincipalEnrollmentSessionStep() {
  const { personManagement, setPersonManagementStep, setSelectedSessionId } = usePrincipalContext()
  const { getClassSessions, getClassEnrollments, getClassById } = usePrincipalData()
  ...
  const classEnrollments = selectedClassId ? getClassEnrollments(selectedClassId) : []

  // 세션별 대기 중인 수강신청 수 계산
  const getSessionPendingCount = (sessionId: number) => {
    return classEnrollments.filter(
      (enrollment) => enrollment.session?.id === sessionId && enrollment.status === 'PENDING'
    ).length
  }
  ...
  const sessionsWithPendingEnrollments = classSessions.filter(
    (session: any) => getSessionPendingCount(session.id) > 0
  )

  if (sessionsWithPendingEnrollments.length === 0) {
    return (
      <div className="space-y-4">
        ...
        <div className="flex flex-col items-center justify-center py-12 text-gray-500">
          ...
          <p className="text-sm">이 클래스의 모든 세션에서 대기 중인 수강신청이 없습니다.</p>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="space-y-4">
      <div>
        <h2 className="text-xl font-semibold">{selectedClass.className}</h2>
        <p className="text-sm text-gray-600">수강신청 대기 세션</p>
      </div>

      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
        {sessionsWithPendingEnrollments.map((session: any) => {
