...
export function PrincipalEnrollmentClassStep() {
  const { setPersonManagementStep, setSelectedClassId } = usePrincipalContext()
  const { pendingEnrollmentClasses } = usePrincipalData()
  ...
  if (pendingEnrollmentClasses.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-12 text-gray-500">
        <BookOpen className="h-12 w-12 mb-4" />
        <h3 className="text-lg font-medium mb-2">대기 중인 수강신청이 없습니다</h3>
        <p className="text-sm">새로운 수강신청이 들어오면 여기에 표시됩니다.</p>
      </div>
    )
  }
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h2 className="text-xl font-semibold">수강신청 대기 클래스</h2>
        <Badge variant="secondary" className="text-sm">
          총 {pendingEnrollmentClasses.length}개 클래스
        </Badge>
      </div>

      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
        {pendingEnrollmentClasses.map((classData) => (
          <Card
