...
import Calendar from '@/app/(dashboard)/dashboard/student/enroll/[month]/date/Calendar'
import DateSelectFooter from '@/app/(dashboard)/dashboard/student/enroll/[month]/date/DateSelectFooter'
...
export function EnrollmentDateStep() {
  ...
  React.useEffect(() => {
    if (selectedClassIds && selectedClassIds.length > 0 && typeof window !== 'undefined') {
      // 선택된 클래스 정보를 가져와서 localStorage에 저장
      // 실제로는 API에서 클래스 정보를 가져와야 하지만, 
      // Calendar 컴포넌트에서 세션 정보를 로드할 때 클래스 정보도 함께 가져올 예정
    ...
    <div className="flex flex-col min-h-screen bg-white font-[Pretendard Variable]">
      ...
      {(!selectedClassIds || selectedClassIds.length === 0) ? (
        ...
        <Calendar 
          ...
          month={selectedMonth || new Date().getMonth() + 1} // Context에서 월 정보 전달
        />
      )}
